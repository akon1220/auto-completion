...
public class BinarySearchAutocomplete implements Autocompletor {
	...
	public List<Term> topMatches(String prefix, int k) {
		...
		PriorityQueue<Term> priorityQueue = new PriorityQueue<>(Collections.reverseOrder(Comparator.comparing(Term::getWeight)));
        for(int i = first; i <= last; i++) {
            priorityQueue.add(myTerms[i]);
        }
        int min = Math.min(k, priorityQueue.size());
        for(int j = 0; j < min; j++){
            top.add(priorityQueue.remove());
        }
        return top;
	}
